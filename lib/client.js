...
function apply(ctx) {
  ...
  ctx.effect(() => {
    ...
    const pending = {
      getSnapshot: () => new Map(
        [...ctx.uiSession.sessionStatus.getSnapshot()].flatMap(([id, status]) => status.pendingInteraction === void 0 ? [] : [[id, status.pendingInteraction]])
      ),
